...
CREATE DATABASE TESTE_ACCENTURE2
GO
USE TESTE_ACCENTURE2
GO
...
-- ADICIONAMENTO DE INDICES
CREATE CLUSTERED INDEX IX_TbCronograma
	on tbCronograma (idfluxo)
GO

ALTER INDEX IX_TbCronograma
	ON TBCRONOGRAMA
	REBUILD WITH(FILLFACTOR = 80)
GO

CREATE NONCLUSTERED INDEX NIX_TbCronograma
	ON tbCronograma (IDContrato, DataFoto)
	INCLUDE(IDFLUXO, TIPO, DATBASE, DATAEVENTO, FOIREALIZADO, PROJETADO, REALIZADO, TaxaCambio, TaxaVariante, Taxa)
GO

CREATE NONCLUSTERED INDEX NIX_TbContrato
	ON tbContrato (CodContrato)
	Include (NomeContrato, tipoinstrumento, Moeda, Indexador, Calendario, Basis, Pais, SeriePreco, DataUltimoFluxo) --ADIÇÃO DO INCLUDE 
GO
...
GO
						...
						CREATE or ALTER PROCEDURE SP_NomeContrato_Indexador
							@NomeContrato VARCHAR(30)
						 AS
...
CREATE or ALTER PROCEDURE SP_ALTERA_CALENDAR
	@IDCONTRATO INT
AS
DECLARE @CALENDARNAME NVARCHAR (200), -- receberá o novo nome do contrato na substring 
		--@IDCONTRATO INT, -- Recebe Id do contrato
...
SET @CALENDARNAME = (SELECT SUBSTRING( c.Calendario, 4, 13) AS CALENDARNAME
						FROM XML_DATA AS X
						INNER JOIN tbContrato AS C
							ON X.IDContrato = C.IDContrato
						WHERE x.IDContrato = @IDCONTRATO 
...
EXEC SP_ALTERA_CALENDAR 175702
GO
